...
// Helper types for form state for PROTA and Promes using textareas
type ProtaFormState = {
...
type PromesFormState = {
  ...
  komponenMingguan_textarea?: string; // Single textarea for structured input for now
};
...
export function CurriculumFormDialog({
  ...
  const renderSpecificFields = () => {
    switch (itemType) {
      ...
      case "PROTA":
        const protaData = formData as ProtaFormState;
        return (
          <>
            <div className="space-y-1">
              <Label htmlFor="year">Tahun Ajaran</Label>
              <Input id="year" name="year" value={(formData as Partial<AnnualProgram>).year || ''} onChange={handleChange} placeholder="cth., 2023/2024" required />
            </div>
            <div className="space-y-1">
